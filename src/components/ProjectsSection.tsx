...
const projects = [
  {
    title: "E-Commerce Platform", featured: true,
  ...
  {
    title: "AI Chat Application", featured: true,
  ...
  return (
    <section id="projects" className="py-24">
      <motion.div variants={container} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}
        ...
        <motion.h2 variants={item} className="font-display font-medium text-section mb-12 gradient-text">Featured Projects</motion.h2>

        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((project) => (
            <motion.div key={project.title} variants={item} className="glass-card glass-card-hover overflow-hidden group">
              <div className="overflow-hidden relative">
                {project.featured && (
                  <span className="absolute top-3 left-3 z-10 gradient-bg text-xs font-display font-medium px-2.5 py-1 rounded-md text-primary-foreground">Featured</span>
                )}
                <motion.img src={project.image} alt={project.title} loading="lazy" className="w-full h-48 object-cover"
              ...
              <div className="p-5">
                ...
                <div className="flex gap-3">
                  ...
                  <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
